"""
GhostOps Model Integration - Gemini API and routing logic.
...
# Import GhostOps agent components
from agents.screen.tools import SCREEN_TOOLS, SCREEN_TOOL_MAP, set_model_name
...
def _format_rapid_history_for_prompt() -> str:
    ...
    for entry in list(_RAPID_CONVERSATION_HISTORY)[-20:]:
        ...
        if role == "user":
            label = "User"
        elif source in {"browser_use", "cua_cli", "cua_vision", "ghostops", "screen_judge"}:
            label = "Agent"
        else:
...
def _format_chain_state_for_prompt(
    ...
    if not chain_steps:
        return (
            ...
            "For action/execution requests ('do X for me', clone/run/open/click/type), do NOT use `invoke_screen_annotator`.\n"
            "Use `invoke_screen_annotator` only for explanation/annotation requests.\n"
    ...
    return (
        ...
        "For action/execution requests, do NOT use `invoke_screen_annotator`.\n"
...
def _normalize_screen_context_payload(
    ...
    if recommended_agent not in {"cua_cli", "cua_vision", "browser", "ghostops", "direct"}:
        recommended_agent = ""
...
async def _run_routed_agent_step(
    ...
    if agent_name == "ghostops":
        screenshot = get_stored_screenshot()
        screen_prompt = SCREEN_SYSTEM_PROMPT + f"\n# User's Request:\n{routing_result.get('query', '')}"
        try:
            screen_result = await model.generate_screen_response(screen_prompt, screenshot)
            screen_summary = _clean_text(
                screen_result.get("summary"),
                "GhostOps completed the visual guidance task.",
                max_len=420,
            )
            return {
                "agent": "ghostops",
            ...
            return {
                ...
                "task": _routing_task_text(routing_result),
                "success": False,
                "message": _clean_text(str(exc), "GhostOps task failed.", max_len=420),
                "source": "ghostops",
            }
    ...
    if agent_name == "browser":
        ...
        browser_agent = BrowserAgent(model_name=screen_model)
        try:
    ...
    if agent_name == "cua_vision":
        ...
        screenshot = get_stored_screenshot()
        vision_agent = VisionAgent(model_name=screen_model)
        task = routing_result.get("task", "")
        try:
...
async def call_gemini(user_prompt: str, rapid_response_model: str, screen_model: str):
    """
    Main entry point - uses two-tier model system:
    1. Rapid response model (router) decides how to handle the request
    2. Routes to appropriate agent: GhostOps, Browser, or Desktop
    """
    model = GeminiModel(
...
class GeminiModel:
    """
    Gemini model wrapper with routing and GhostOps capabilities.

    Two-tier system:
    - Router model: Fast, no image, decides where to route requests
    - GhostOps model: Full capabilities, with screenshot, for screen annotations

    Documentation Reference: https://github.com/googleapis/python-genai
    """

    def __init__(self, screen_model='gemini-3-flash-preview', rapid_response_model='gemini-flash-lite-latest'):
        ...
        # Config for GhostOps model (full capabilities)
        self.screen_config = types.GenerateContentConfig(
            temperature=1.2,
            top_p=0.95,
            top_k=64,
    ...
    async def route_request(self, prompt: str) -> dict:
        ...
        for function_call in function_calls:
            ...
            if function_call.name == "invoke_screen_annotator":
                return {
                    "agent": "ghostops",
    ...
    async def generate_screen_context(
        ...
        judge_prompt = (
            ...
            '  "recommended_agent": "cua_cli|cua_vision|browser|ghostops|direct",\n'
