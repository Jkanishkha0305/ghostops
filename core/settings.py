...
def get_model_configs(settings_path: str = DEFAULT_SETTINGS_PATH) -> tuple[str, str]:
    """Get both model configurations: (rapid_response_model, screen_model)"""
    with open(settings_path, "r", encoding="utf-8") as handle:
        settings = json.load(handle)
    return (
        settings.get("rapid_response_model", "gemini-flash-lite-latest"),
        settings.get("screen_model", "gemini-3-flash-preview")
    )
...
def get_screen_model(settings_path: str = DEFAULT_SETTINGS_PATH) -> str:
    with open(settings_path, "r", encoding="utf-8") as handle:
        settings = json.load(handle)
    return settings.get("screen_model", "gemini-3-flash-preview")
