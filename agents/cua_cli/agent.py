...
class CLIAgent:
    ...
    def _build_cli_env(self) -> Dict[str, str]:
        ...
        # Enable permissive policy for GhostOps CLI sessions:
        # allow all tools by default while still blocking dangerous shell commands.
        env["GhostOps_CLI_PERMISSIVE_POLICY"] = "1"
        ...
        # Disable sandbox for maximum tool/file access in GhostOps CLI sessions.
    ...
    def _ensure_trusted_folders_config(self) -> str:
        ...
        """
        trusted_file = Path(tempfile.gettempdir()) / "ghostops_gemini_trusted_folders.json"
        entries = {
    ...
    def _ensure_gemini_cli_home(self) -> str:
        ...
        """
        gemini_home = Path(self.gemini_cli_path).resolve() / ".ghostops_gemini_home"
    ...
    def _prepare_cli_task(task: str) -> str:
        ...
        """
        instruction = (
            "You are running inside GhostOps with tool access enabled. "
    ...
    async def _start_background_process(
        ...
        log_path = os.path.join(tempfile.gettempdir(), f"ghostops_cli_bg_{process_id}.log")
